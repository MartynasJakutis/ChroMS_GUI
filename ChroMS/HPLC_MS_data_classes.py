...
class HPLC_3D_Data(HPLC_MS_Data):
    ...
    def __init__(self, file, wave_nm):
        super().__init__(file)
    ...
    def read(self):
        ...
        self.wavelenghts = (np_array(data_list[0]) // 100)
        
        wv_index = (np_where(self.wavelenghts == self.wave_nm)[0])[0]

        data_array = np_array(data_list[1:])
        if data_array.dtype == np_float64:
            self.retention_time = data_array[:, 0]
            self.all_ab_intensities = data_array[:, 1:] / 1000
